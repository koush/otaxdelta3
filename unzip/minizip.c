...
int add_to_zip(zipFile zf, int opt_exclude_path, int opt_compress_level, int opt_recurse, const char* password, void* buf, int size_buf, const char* filenameinzip) {
    int err=ZIP_OK;
    if (!((((*(filenameinzip))=='-') || ((*(filenameinzip))=='/')) &&
          ((filenameinzip[1]=='o') || (filenameinzip[1]=='O') ||
          (filenameinzip[1]=='a') || (filenameinzip[1]=='A') ||
          (filenameinzip[1]=='r') || (filenameinzip[1]=='R') ||
           (filenameinzip[1]=='p') || (filenameinzip[1]=='P') ||
           ((filenameinzip[1]>='0') || (filenameinzip[1]<='9'))) &&
          (strlen(filenameinzip) == 2)))
    {
        struct stat cst;
        int ret;
        if (opt_recurse) {
            if ((ret = lstat(filenameinzip, &cst))) {
                fprintf(stderr, "Error opening: %s\n", filenameinzip);
                return ZIP_ERRNO;
            }
            if (S_ISDIR(cst.st_mode)) {
                DIR *dp = opendir(filenameinzip);
                if (dp == NULL) {
                    fprintf(stderr, "Error opening directory: %s\n", filenameinzip);
                    return ZIP_ERRNO;
                }
